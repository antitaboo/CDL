...
(***
Note that actually here we compute all occurrences of variables in a cdl formula, not really all variables, because
...
Fixpoint E_exp_FV (e : E_exp) (bv : list Var) : list Var := (*bv : list of bounded variables at current time *)
  ...
  | ECntClkC c => if (NotIn_bool c bv) then (c :: nil) else nil
  | EStClkC c => if (NotIn_bool c bv) then (c :: nil) else nil
  | EnumC n => nil
...
Fixpoint EvtE_FV (e : EvtElement) (bv : list Var) : list Var :=
  match e with 
  | sigC s e' => s :: (e_exp_FV e' bv)
  | assC s e' => e_exp_FV e' (s :: bv)
  end
.
...
Fixpoint EvtE_BV (e : EvtElement) : list Var :=
  match e with 
  | sigC s e' => nil
...
Fixpoint Evt_chk (evt : Evt) (ev : list Var) (x : Var) (bv : list Var) : Prop :=
  match evt with 
  | nil => nil
  | e :: evt' => (EvtE_chk e ev x bv) /\ (Evt_chk e ev x ((EvtE_BV e nil) :: bv))
  end
.
...
Fixpoint cdl_chk (e : CDL_exp) (ev : list Var) (x : Var) (bv : list Var) : Prop := 
    ...
    | cdl_box2C p e' => (SEP_chk p ev x bv) /\ (cdl_chk e' ev x (bv ++ (SEP_exp_BV p nil)))
    ...
    | cdl_dia2C p e' => (SEP_chk p ev x bv) /\ (cdl_chk e' ev x (bv ++ (SEP_exp_BV p nil)))
