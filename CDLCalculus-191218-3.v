...
Inductive tstSeq : Gamma -> place -> place -> Delta -> Type :=
  tst1 : forall (T : Gamma) (D : Delta) (e : e_exp) (phi : CDL_exp), 
          (
          forall (x' : string), (NotIn x' (cdl_FV_l T nil)) -> (NotIn x' (cdl_FV_l D nil)) -> (NotIn x' (cdl_FV ([@ {x :=' e} ] phi) nil)) ->
          (tstSeq ( (x =' (e_2_E e) [x' // x] :: T) [[ x' // x ]]) (<< >>) (<< phi >>) (D [[ x' // x ]]) )
        ) -> 
    (tstSeq T (<< >>)(<< [ @ {x :=' e} ] phi >>) D)
.
...
Axiom tst3 : forall (B : nat -> Type), True -> (forall (y : nat), B y).

Inductive C : nat -> Type :=
  dd : C 0.

Theorem thm1 : forall (z : nat), C z.
Proof. 
intro z. 
apply tst3. 
