...
Notation "v( x )" := (tVarC x) (at level 52).
Notation "vn( c )" := (tCntC c) (at level 52).
Notation "vs( c )" := (tStC c) (at level 52).
...
Compute E_exp_subsE (x +' y) (y +' 1) (v(x)).
Compute E_exp_subsE (x *' z) (y *' 5) (v(c)).
Compute E_exp_subsE (n(c) *' z) (y *' 5) (vn(c)).
...
Compute (z '- 5) '* x '+ z.
Compute e_exp_subse ((z '- 5) '* x '+ z) y (v(z)).
...
Fixpoint E_exp_chk (e : E_exp) (ev : list Var) (x : Var) (bv : list Var) : Prop := 
  ...
  match e with
  | EvarC v => if andb (eqv v x) (commonEle ev bv) then False else True
  | ECntClkC c => True (* since we now consider substitution for a Var *)
  | EStClkC c => True
  | EnumC n => True
  | EplusC e1 e2 => (E_exp_chk e1 ev x bv) /\ (E_exp_chk e2 ev x bv)
  | EmulC e1 e2 => (E_exp_chk e1 ev x bv) /\ (E_exp_chk e2 ev x bv)
  | EminusC e1 e2 => (E_exp_chk e1 ev x bv) /\ (E_exp_chk e2 ev x bv)
  | EdivC e1 e2 => (E_exp_chk e1 ev x bv) /\ (E_exp_chk e2 ev x bv)
  end
.
...
Fixpoint e_exp_chk (e : e_exp) (ev : list Var) (x : Var) (bv : list Var) : Prop :=
  match e with
  | enumC n => True
  | evarC s => if andb (eqv s x) (commonEle ev bv) then False else True
  | eplusC e1 e2 => (e_exp_chk e1 ev x bv) /\ (e_exp_chk e2 ev x bv)
  | eminusC e1 e2 => (e_exp_chk e1 ev x bv) /\ (e_exp_chk e2 ev x bv)
  | emulC e1 e2 => (e_exp_chk e1 ev x bv) /\ (e_exp_chk e2 ev x bv)
  | edivC e1 e2 => (e_exp_chk e1 ev x bv) /\ (e_exp_chk e2 ev x bv)
...
Fixpoint EvtE_chk (e : EvtElement) (ev : list Var) (x : Var) (bv : list Var) : Prop :=
  match e with 
  | sigC s e' => e_exp_chk e' ev x (s :: bv)
  | assC s e' => e_exp_chk e' ev x (s :: bv)
  end
.


Fixpoint Evt_chk (evt : Evt) (ev : list Var) (x : Var) (bv : list Var) : Prop :=
  match evt with 
  | nil => True
  | e :: evt' => (EvtE_chk e ev x bv) /\ (Evt_chk evt' ev x (bv ++ (EvtE_BV e nil)))
  end
.


Fixpoint P_exp_chk (P : P_exp) (ev : list Var) (x : Var) (bv : list Var) : Prop :=
  match P with
  | PtrueC => True
  | PlteC e1 e2 => (e_exp_chk e1 ev x bv) /\ (e_exp_chk e2 ev x bv)
  | PnegC P' => P_exp_chk P' ev x bv
  | PandC P1 P2 => (P_exp_chk P1 ev x bv) /\ (P_exp_chk P2 ev x bv)
  | PfalseC => True
  | PltC e1 e2 => (e_exp_chk e1 ev x bv) /\ (e_exp_chk e2 ev x bv)
  | PgtC e1 e2 => (e_exp_chk e1 ev x bv) /\ (e_exp_chk e2 ev x bv)
  | PgteC e1 e2 => (e_exp_chk e1 ev x bv) /\ (e_exp_chk e2 ev x bv)
  | PeqC e1 e2 => (e_exp_chk e1 ev x bv) /\ (e_exp_chk e2 ev x bv)
  | PorC P1 P2 => (P_exp_chk P1 ev x bv) /\ (P_exp_chk P2 ev x bv)
  | PimpC P1 P2 => (P_exp_chk P1 ev x bv) /\ (P_exp_chk P2 ev x bv)
  end
.



Fixpoint SEP_exp_chk (p : SEP_exp) (ev : list Var) (x : Var) (bv : list Var) : Prop :=
  match p with 
  | skip => True
  | evtC evt => Evt_chk evt ev x bv
  | tstC P evt => (P_exp_chk P ev x bv) /\ (Evt_chk evt ev x bv) (* it is impossible to have any 
    new bounded variables in P *)
  | seqC e1 e2 => (SEP_exp_chk e1 ev x bv) /\ (SEP_exp_chk e2 ev x (bv ++ (SEP_exp_BV e1 nil)))
  | choC e1 e2 => (SEP_exp_chk e1 ev x bv) /\ (SEP_exp_chk e2 ev x bv)
  | loopC e' => SEP_exp_chk e' ev x bv
  end
.
...
Fixpoint cdl_chk (e : CDL_exp) (ev : list Var) (x : Var) (bv : list Var) : Prop := 
    ...
    | cdl_box1C p r => SEP_exp_chk p ev x bv
    | cdl_box2C p e' => (SEP_exp_chk p ev x bv) /\ (cdl_chk e' ev x (bv ++ (SEP_exp_BV p nil)))
    ...
    | cdl_dia1C p r => SEP_exp_chk p ev x bv
    | cdl_dia2C p e' => (SEP_exp_chk p ev x bv) /\ (cdl_chk e' ev x (bv ++ (SEP_exp_BV p nil)))
...
(* Rule (Phi) *)
  r_Phi_all_int1 : forall (T : Gamma) (D : Delta) (V : list Var) (x : Var) (e : e_exp) (A : Evt) (phi : CDL_exp), 
          let n := NewId V in 
          (
          ...
          ->
          (
            Seq T empty (exp ([ @ ((x :=' e) :: A) ] phi)) D V
          )
|

  r_Phi_all_int2 : forall (T : Gamma) (D : Delta) (V : list Var) (phi : CDL_exp), 
          (
            Seq T empty (exp phi) D V
          ) 
          -> 
          (
            Seq T empty (exp ([ @ idle ] phi)) D V
          )
|

(* Rule (Pi) *)
  r_Pi_all_int1 : forall (T : Gamma) (D : Delta) (V : list Var) (x : Var) (e : e_exp) (A : Evt) (r : rel), 
          let n := NewId V in 
          (
            Seq ((x =' (e_2_E e) [(var n) subs-E x]) :: (T [ (var n) subs-l x ])) 
                      empty 
          ...
          ->
          (
            Seq T empty (exp ([ @ ((x :=' e) :: A) ]' r)) D V
          )
|
  r_Pi_all_int2 : forall (T : Gamma) (D : Delta) (V : list Var) (r : rel), 
          (
            Seq T empty (exp (hbar r)) D V
          ) 
          -> 
          (
            Seq T empty (exp ([ @ idle ]' r)) D V
          )
|

(* Rule P? *)
  r_Test_all_phi_int : forall (T : Gamma) (D : Delta) (V : list Var) (P : P_exp) (A : Evt) (phi : CDL_exp), 
          (
            Seq T empty (exp ((Pexp_2_CDLexp P) ->' [ @ A ] phi )) D V
          )
          ->
          (
            Seq T empty (exp ([ P ? A ] phi)) D V
          )
|
  r_Test_all_rel_int : forall (T : Gamma) (D : Delta) (V : list Var) (P : P_exp) (A : Evt) (r : rel), 
          (
            Seq T empty (exp ((Pexp_2_CDLexp P) ->' [ @ A ]' r )) D V
          )
          ->
          (
            Seq T empty (exp ([ P ? A ]' r)) D V
          )
|

(* Rule \pi\epsilon *)
  r_PiEp_all_int : forall (T : Gamma) (D : Delta) (V : list Var) (r : rel), 
          (
            Seq T empty ( exp (tt') ) D V
          )
          ->
          (
            Seq T empty (exp ([ skip ]' r)) D V
          )
|

(* Rule \epsilon *)
  r_Ep_all_int : forall (T : Gamma) (D : Delta) (V : list Var) (phi : CDL_exp), 
          (
            Seq T empty ( exp phi ) D V
          )
          ->
          (
            Seq T empty (exp ([ skip ] phi)) D V
          )
|

(* Rule \pi[;] *)
  r_PiSeq_all_int : forall (T: Gamma) (D : Delta) (V : list Var) (p : SEP_exp) (q : SEP_exp) (r : rel),
          (
            Seq T empty
                    (exp ([ p ]' r /\' [ p ] ([ q ]' r)))
                      D V
          ...
          ->
          (
            Seq T empty (exp ([ p;q ]' r)) D V
          )
|

(* Rule \pi[\cup] *)
  r_PiCho_all_int : forall (T: Gamma) (D : Delta) (V : list Var) (p : SEP_exp) (q : SEP_exp) (r : rel),
          (
            Seq T empty
                    (exp ([ p ]' r /\' [ q ]' r))
                      D V
          ...
          ->
          (
            Seq T empty (exp ([ p U q ]' r)) D V
          )
|

(* Rule \pi[*]u *)
  r_PiLoopU_all_int : forall (T: Gamma) (D : Delta) (V : list Var) (p : SEP_exp) (r : rel),
          (
            Seq T empty
                    (exp ([ p ; p ** ]' r))
                      D V
          ...
          ->
          (
            Seq T empty (exp ([ p ** ]' r)) D V
          )
|

(* Rule \pi[*]i *)
  r_PiLoopI_all_int : forall (T: Gamma) (D : Delta) (V : list Var) (p : SEP_exp) (r : rel),
          (
            Seq T empty
                    (exp ([ p ** ] ([ p ]' r)))
                      D V
...
(* Rule [;] *)
  r_Seq_all_int : forall (T: Gamma) (D : Delta) (V : list Var) (p : SEP_exp) (q : SEP_exp) (phi : CDL_exp),
          (
            Seq T empty
                    (exp ([ p ] ([ q ] phi)))
                      D V
          ...
          ->
          (
            Seq T empty (exp ([ p ; q ] phi)) D V
          )
|

(* Rule [U] *)
  r_Cho_all_int : forall (T: Gamma) (D : Delta) (V : list Var) (p : SEP_exp) (q : SEP_exp) (phi : CDL_exp),
          (
            Seq T empty
                    (exp ([ p ] phi /\' [ q ] phi))
                      D V
          ...
          ->
          (
            Seq T empty (exp ([ p U q ] phi)) D V
          )
|

(* Rule [*]u *)
  r_LoopU_all_Int : forall (T: Gamma) (D : Delta) (V : list Var) (p : SEP_exp) (phi : CDL_exp),
          (
            Seq T empty
                    (exp (phi /\' [ p ; p ** ] phi))
                      D V
...
(* Rule [*]i *)
  r_LoopI_all : forall (T : Gamma) (D : Delta) (V : list Var) (p : SEP_exp) (phi : CDL_exp), 
          (
            exists (inv : CDL_exp), (* invariant has to be manually provided *)
              (
                Seq T empty (exp inv) D (V ++ (cdl_V inv V))
              )
              /\
              (
                Seq nil empty 
...
(* Rule <*>i *)
  r_LoopI_dia : forall (T : Gamma) (D : Delta) (V : list Var) (p : SEP_exp) (phi : CDL_exp), 
          (
...
(* ****************************** rules of FOL *********************** *)
(* Rule (o) *)
  r_o_int : forall (T : Gamma) (D : Delta) (V : list Var),
        (
        ...
        ->
        (
          Seq T empty empty D V
        )
|

(* Rule (ax) *)
  r_ax_int : forall (T : Gamma) (D : Delta) (V : list Var) (phi : CDL_exp), 
        (
          True
        )
        ->
        (
          Seq T (exp phi) (exp phi) D V
        )
|

(* Rule (cut) *)
  r_cut_int : forall (T : Gamma) (D : Delta) (V : list Var), 
        (
          forall (phi : CDL_exp),
            (
              Seq T empty (exp phi) D (V ++ (cdl_V phi V))
            )
            /\
            (
              Seq T (exp phi) empty D (V ++ (cdl_V phi V))
            )
        )
        ->
        (
...
(* Rule (~ r) *)
  r_negR_int : forall (T : Gamma) (D : Delta) (V : list Var) (phi : CDL_exp),
        (
          Seq T (exp (~' phi)) empty D V
        )
        ->
        (
          Seq T empty (exp phi) D V
...
(* Rule (~ l) *)
  r_negL_int : forall (T : Gamma) (D : Delta) (V : list Var) (phi : CDL_exp),
        (
          Seq T empty (exp (~' phi)) D V
        )
        ->
...
(* Rule (/\ r) *)
  r_andR_int : forall (T : Gamma) (D : Delta) (V : list Var) (phi1 : CDL_exp) (phi2 : CDL_exp),
        (
          (
            Seq T empty (exp phi1) D V
          )
          /\
          (
            Seq T empty (exp phi2) D V
          )
        )
        ->
        (
          Seq T empty (exp (phi1 /\' phi2)) D V
        )
|

(* Rule (/\ l) *)
  r_andL_int : forall (T : Gamma) (D : Delta) (V : list Var) (phi1 : CDL_exp) (phi2 : CDL_exp),
        (
          Seq (phi2 :: T) 
                (exp phi1) empty D V
        )
        ->
        (
          Seq T (exp (phi1 /\' phi2)) empty D V
        )
|

(* Rule (all r) *)
  r_allR_int : forall (T : Gamma) (D : Delta) (V : list Var) (phi : CDL_exp) (x : Var),
        (
          let n := NewId V in
          (
            Seq T empty 
        ...
        ->
        (
          Seq T empty (exp (all x , phi)) D V
        )
|

(* Rule (all l) *)

  r_allL_int : forall (T : Gamma) (D : Delta) (V : list Var) (phi : CDL_exp) (x : Var),
        (
          exists (E : E_exp),
          (
        ...
        ->
        (
          Seq T (exp (all x , phi)) empty D V
        )
|

(* Rule (\/ r) *)
  r_orR_int : forall (T : Gamma) (D : Delta) (V : list Var) (phi1 : CDL_exp) (phi2 : CDL_exp),
        (
          Seq T empty (exp phi1) ((phi2) :: D) V
        )
        ->
...
(* Rule (\/ l) *)
  r_orL_int : forall (T : Gamma) (D : Delta) (V : list Var) (phi1 : CDL_exp) (phi2 : CDL_exp),
        (
          (
            Seq T (exp phi1) empty D V
          )
          /\
          (
            Seq T (exp phi2) empty D V
          )
        )
        ->
        (
...
(* Rule (-> r) *)
  r_impR_int : forall (T : Gamma) (D : Delta) (V : list Var) (phi1 : CDL_exp) (phi2 : CDL_exp),
        (
          Seq T (exp phi1) (exp phi2) D V
        )
        ->
        (
          Seq T empty (exp (phi1 ->' phi2)) D V
...
(* Rule (-> l) *)
  r_impL_int : forall (T : Gamma) (D : Delta) (V : list Var) (phi1 : CDL_exp) (phi2 : CDL_exp),
        (
          (
            Seq T empty (exp phi1) D V
          )
...
(* Rule (ext r) *)
  r_extR_int : forall (T : Gamma) (D : Delta) (V : list Var) (phi : CDL_exp) (x : Var),
        (
          exists (E : E_exp),
            (cdl_chk phi (E_exp_V E nil) x nil) (* make sure that the substitution phi[E / x] is admissible *)
            /\
        ...
        ->
        (
          Seq T empty (exp (ext x , phi)) D V
        )
|

(* Rule (ext l) *)
  r_extL_int : forall (T : Gamma) (D : Delta) (V : list Var) (phi : CDL_exp) (x : Var),
        (
          let n := NewId V in
          (
            Seq ((ext x, phi) :: T) 
        ...
        ->
        (
          Seq T (exp (ext x , phi)) empty D V
        )

(* define the notation at the end *)
where "{ T , p1 ==> p2 , D & V }" := (Seq T p1 p2 D V)
.
